...
def get_impact_row(x, y, mean_x, std_x, mean_y, std_y,
                   gbm_temp, gbm_slope,
                   name_feature):
    ...
    for key, val in impact_example.items():
        print(key)
        print("%.2f" % val[0][0])
        list_impact.append(round(val[0][0], 0))
        impact_sum += val[0][0]
    print("sum of impact:", round(impact_sum, 2))
    print("actual impact:", round(impact_actual[0][0], 2))
    y_norm_pred = np.transpose( np.concatenate([[gbm_temp.predict(x_eg, num_iteration=gbm_temp.best_iteration_)],
                                           [gbm_slope.predict(x_eg, num_iteration=gbm_slope.best_iteration_)]]))
    y_pred = y_norm_pred * std_y + mean_y
    return y_pred, list_impact
...
if __name__ == "__main__":
    ...
    for name in name_feature:
        print(name)
    # get row from the csv
    path_data = "../data_raw/sequence.csv"
    x_data, y_data = load_raw_data(path_data)
    data = pd.read_csv(path_data).values
    ## name of fields
    name_field = list(pd.read_csv("../data/train+excluded9heat_no.csv").columns) + ['time_mid']
    for feature in name_feature:
        name_field.append('impact ' + feature)
    print(name_field)
    with open("../data_raw/sequence_impact.csv", 'w', newline='') as file_impact:
        csvwriter = csv.writer(file_impact)
        csvwriter.writerow(name_field)
        for ind in range(len(x_data)):
            y_pred, list_impact = get_impact_row(np.asarray([x_data[ind]]), np.asarray([y_data[ind]]),
            ...
            row = data[ind].copy()
            row[-2:] = y_pred[:]
            row = np.concatenate((row, [None],  list_impact), axis=0)
            print(row)
