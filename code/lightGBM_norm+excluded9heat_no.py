...
def plot_importance(importance_feature, name_feature):
    ...
    plt.tight_layout()
    plt.savefig('./lightGBM_importance+excluded9heat_no.png')
    plt.show()
...
if __name__ == "__main__":
    ...
    list_temp_mid_test_worst = [(heat_no, temp_pred, temp_actual) for (heat_no, temp_pred, temp_actual) in list(zip(list_heat_no_test, y_test_pred[:, 0].ravel(), y_test_raw[:, 0].ravel()))
                           if heat_no in list_heat_no_test_worst]
    print(list_temp_mid_test_worst)
    list_heat_no_test_best = [(heat_no, temp, slope * 60) for (heat_no, error, temp, slope) in list(zip(list_heat_no_test, list_error_temp_test, y_test_pred[:, 0].ravel(), y_test_pred[:, 1].ravel()))
                               if abs(error) < 0.25]
    print("best")
    print(list_heat_no_test_best)
    '''save and load models'''
    joblib.dump(gbm_temp , './lightGBM_temp+excluded9heat_no.pkl')
    joblib.dump(gbm_slope, './lightGBM_slope+excluded9heat_no.pkl')
    gbm_temp_load  = joblib.load('./lightGBM_temp+excluded9heat_no.pkl')
    gbm_slope_load = joblib.load('./lightGBM_slope+excluded9heat_no.pkl')
