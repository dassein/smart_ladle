...
def plot_importance(importance_feature, name_feature):
    ...
    plt.tight_layout()
    plt.savefig('./lightGBM_importance+excluded9heat_no_nonlinear.png')
    plt.show()


if __name__ == "__main__":
    '''prepare dataset'''
    x_train_raw, y_train_raw = load_raw_data("../data/train+excluded9heat_no_nonlinear.csv")
    x_val_raw  , y_val_raw   = load_raw_data("../data/val_nonlinear.csv")
    x_test_raw , y_test_raw  = load_raw_data("../data/test_nonlinear.csv")
    ...
    df_val = pd.read_csv('../data/val_nonlinear.csv')
    ...
    df_test = pd.read_csv('../data/test_nonlinear.csv')
    ...
    joblib.dump(gbm_temp , './lightGBM_temp+excluded9heat_no_nonlinear.pkl')
    joblib.dump(gbm_slope, './lightGBM_slope+excluded9heat_no_nonlinear.pkl')
    gbm_temp_load  = joblib.load('./lightGBM_temp+excluded9heat_no_nonlinear.pkl')
    gbm_slope_load = joblib.load('./lightGBM_slope+excluded9heat_no_nonlinear.pkl')
    '''plot the importance of variables'''
    importance_feature = list(gbm_temp.feature_importances_)
    print(importance_feature)
    name_feature = list(pd.read_csv("../data/train+excluded9heat_no_nonlinear.csv").columns[: -2])
    print(name_feature)
    plot_importance(importance_feature, name_feature)
    '''export mean, std of x, y'''
    print(mean_x)
    print(std_x)
    print(mean_y)
    print(std_y)