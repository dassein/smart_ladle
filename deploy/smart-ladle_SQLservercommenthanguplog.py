...
import csv
import lightgbm
import json  # to dump out args of program
...
def extract_interval_relineflag(heat_no, table, cursor):
    ...
    sql_query_time_current = \
        "select top 1 t_stamp, ladle_no from %s \
        where heat_no = ? \
        order by t_stamp desc;" % (table)  # Get latest time record of heat_no
    ...
    sql_query = \
        "select distinct top 4 heat_no, max(t_stamp) t_last from %s \
        where t_stamp <= ? and heat_no is not null and heat_no != '        ' and ladle_no = ?  \
        group by heat_no \
        order by t_last desc;" % (table)  # Get latest 4 heat_no
    ...
    sql_query_time_start = \
        "select top 1 t_stamp, ladle_no, location_code from %s \
        where heat_no = ? order by t_stamp;" % (table)  # find 1st record
    ...
    for i, time_steel_start in enumerate(list_time_steel_start):
        ...
        sql_query_part = \
            "select t_stamp, location_code, event_code from %s \
            where t_stamp >= ? and t_stamp < ? and ladle_no = ? order by t_stamp;" % (table)  # edited by Zhankun, July 06, 2020
        ...
        if time_steel_end == False:
            print(str(list_heat_no[i]) + "doesn't LADLE_SPIN_OUT (no event_code==18)" + "for ladle_no=" + str(ladle_no))
    ...
    sql_query_time_reline = \
        "select top 1 t_stamp from %s \
        where t_stamp < ? and ladle_no = ? and location_code = 25 \
        order by t_stamp desc;" % (table)  # location_code = 25 represent TEAR_OUT (means reline)
    ...
    sql_query_reline_flag = \
        ...
        and event_code = 17;" % (table)  # 17 => LADLE_SPIN_IN, revised by Zhankun, April 23, 2021
    ...
    flag_in_LMF = False
    sql_query_in_LMF = \
        "select top 1 location_code from %s \
        where t_stamp < ? and heat_no = ? \
        order by t_stamp desc;" % (table)
...
def extract_replaceflag(heat_no, table, cursor):
    ...
    sql_query_time_current = \
        "select top 1 t_stamp, location_code from %s \
        where heat_no = ? and str_value in ('%s')  \
        order by t_stamp desc;" % (table, '\',\''.join(str for str in str_heat))
    cursor.execute(sql_query_time_current, (heat_no))
    ...
    sql_query_time_replace = \
        ...
        order by t_stamp desc;" % (table, '\',\''.join(str for str in str_seq_start) )
    ...
    sql_query_replace_flag = \
        ...
        and str_value in ('%s');" % (table, '\',\''.join(str for str in str_heat) )
    ...
    if replace_flag != 2:
        sql_query_replace_ornot1 = \
            "select top 1 heat_no from %s \
            where t_stamp < ? and t_stamp >= ? and location_code = ? \
            and str_value = '%s' \
            order by t_stamp desc;" % (table, str_heat[1]) # verify last start with empty heat_no ?
...
def extract_EAF(heat_no, table, cursor):
    sql_query_EAF = \
        "select top 1 tap_temp_ts, tap_temp, tap_weight from %s \
        where heat_no = ? order by tap_temp_ts desc;" % (table)  # find 1st record
...
def extract_LMF(heat_no, table, cursor):
    sql_query_LMF_end = \
        "select top 1 t_stamp, num_value from %s \
        where heat_no = ? and str_value = 'TEMPERATURE_SAMPLE'\
        order by t_stamp desc;" % (table)  # find 1st record
...
def extract_CAST(heat_no, table, cursor):
    sql_query_current = \
        "select top 1 t_stamp from %s \
        where heat_no = ? \
        order by t_stamp desc;" % (table) # find the latest record, make sure tundish start wthin 1 hour
    ...
    sql_query = \
        "select top 1 t_stamp, tundish_temp from %s \
...
def predict_CAST(location_code, table, cursor):
    ...
    sql_query_weight_speed = \
        "select top 1 heat_no, cast_weight, tundish_temp, cast_speed from %s \
        where t_stamp < ? and location_code = ? \
        order by t_stamp desc;" % (table)
    ...
    sql_query_dT_dt = \
        "select top 200 t_stamp, tundish_temp, cast_weight from %s \
...
def extract_temp_liquidus(heat_no, table, cursor):
    sql_query = "select top 1 cast_liquidus from %s \
                where heat_no = ? order by t_stamp desc;" % (table)
...
def predict_temp_liquidus(location_code, table, cursor):
    sql_query = "select top 1 cast_liquidus from %s \
                where location_code = ? order by t_stamp desc;" % (table)
...
def extract_deviation(heat_no, table, cursor):
    ...
    if not repr_int(heat_no_prev):
        return -1
    sql_query_weight_speed = \
        "select top 1 cast_weight, t_stamp from %s \
    ...
    sql_query_temp_mid = \
        "select top 1 tundish_temp, cast_max_temp_speed, cast_min_temp_speed from %s \
...
def extract_temp_mix_min(heat_no, table, cursor):
    sql_query_temp = \
        "select top 1 cast_max_temp_speed, cast_min_temp_speed from %s \
        where heat_no = ?  \
        order by t_stamp desc;" % (table)
...
def extract_time_mid(heat_no, table, cursor):
    ...
    if len(result) == 0: # lastest heat_no which is casting
        sql_query_weight_speed = \
            "select top 1 t_stamp, cast_weight from %s \
            where t_stamp >= ? and heat_no = ? \
            order by t_stamp desc;" % (table)
...
def grab_current_data(heat_no, location_code):
    ...
    time_mid = time2str(str2time(time_tundish_start) + timedelta(seconds=dt_mid))
    print("... calculated midpoint time")
    print("Completed grabbing current heat data")
...
def grab_prev_data(heat_no, location_code):
    ...
    sql_query_time_current = \
        "select top 1 t_stamp, location_code from %s \
        where heat_no = ? and str_value in ('%s')  \
        order by t_stamp desc;" % (table_caster_event, '\',\''.join(str for str in str_heat_all))
    cursor.execute(sql_query_time_current, (heat_no))
    result = cursor.fetchall()
    ...
    if location_code == 13: # after specify the location
        ...
        raise Exception("location_code wrong")
    print("... starting second SQL query")
    sql_query_time_replace = \
        "select top 1 t_stamp from %s \
        where location_code = ? and str_value in ('%s') and t_stamp < ? \
        order by t_stamp desc;" % (table_caster_event, '\',\''.join(str for str in str_seq_start))
    cursor.execute(sql_query_time_replace, (location_code, time_current))
    time_replace = cursor.fetchall()[0][0]
    sql_query_time_prev = \
        "select top 1 heat_no, t_stamp from %s \
        where location_code = ? and t_stamp < ? \
        and str_value in ('%s') \
        order by t_stamp desc;" % (table_caster_event, '\',\''.join(str for str in str_heat))
    cursor.execute(sql_query_time_prev, (location_code, time_current))
    print("... second SQL query complete")
    ...
    time_interval_LMF = delta_time(time_LMF_end, time_LMF_start)
    time_interval_steel = time_interval_EAF2LMF + time_interval_LMF
    print("... calculated time intervals")
    list_data = [heat_no, ladle_no, location_code] + [reline_flag, replace_flag] + \
                [time_interval_LMF2CAST, temp_LMF_end] + list_interval + [time_interval_steel, throughput] + \
                [time_interval_EAF2LMF, time_interval_LMF, temp_EAF, temp_LMF_start, temp_tundish_start] + \
                [temp_liquidus, deviation]
    print("... compiled list data")
    ...
    print("... extracted temperature limits")
    print("Completed extracting prev heat data")
    return list_data, dt_mid, time_mid, cast_max_temp_speed, cast_min_temp_speed
...
def grab_next_data(heat_no, location_code):
    ...
    sql_query_time_next = \
        ...
        where location_code = ? and t_stamp > ? \
        and str_value in ('%s') \
        order by t_stamp;" % (table_caster_event, '\',\''.join(str for str in str_heat))
    ...
    cast_max_temp_speed, cast_min_temp_speed = extract_temp_mix_min(heat_no, table_caster_process, cursor)
    print("... extracted temperature limits")
    print("Completed extracting next heat data")
    return list_data, dt_mid, time_mid, cast_max_temp_speed, cast_min_temp_speed
...
def extract_actual(heat_no, table, cursor):
    print("Extracting caster temperature data...")
    sql_query_time_current = \
        "select top 1 t_stamp from %s \
        where heat_no = ? \
        order by t_stamp desc;" % (table)
...
if __name__ == "__main__":
    ...
    server = 'laptop_name\SQLEXPRESS'  # '11.11.11.11'
    database = 'database'  
    username = 'root' 
    password = 'password'
    cxnstring = "DRIVER={" + driver + "};SERVER=" + server + ";DATABASE=" + database + \
    ...
    table_ladle_event = '[1-ladle_event]'  # 'ladle_event'
    table_caster_event = '[3-caster_event]'  # 'caster_event'
    table_EAF = '[4-eaf]'  # 'eaf_heat_data'
    table_LMF_event = '[2-lmf_event]'  # 'lmf_event'
    table_caster_process = '[caster_process]'
    ...
    parser.add_argument('redundant', help="redundant arg")  # to match the redundant arg '/k` from unity
    ...
    # dump out the args to json file
    dict_args = {"heat_no": args.heat_no, "caster_no": args.caster_no, "prediction": args.prediction,
            "direction": args.direction}
    json = json.dumps(dict_args)
    f = open(root + "args_dict.json", "w")
    f.write(json)
    f.close()
    ...
    print("Preparing machine learning algorithm...")
    ...
    if args.prediction:
        ...
        print("Begin previous heat processing...")
        ...
        print("... data mapped and normalized")
        ...
        print("... generated predictions and plots")
        ...
        print("... prepared CSV inputs")
        dict_output = {'temp_mid': round(y_pred[0][0]), 'slope_mid': round(y_pred[0][1] * 60, 2), 'time_mid': time_mid}
        print("... prepared CSV outputs")
        dict_row = {**dict_input, **dict_output, **dict_impact}
        print("... prepared CSV impacts")
        print("... opening CSV file for writing at",root+"output.csv")
        with open(root + "output.csv", 'w', newline='') as csvfile:
        ...
        with open(root + "output_log.csv", 'a+', newline='') as csvfile:  # save logging file
            ...
            csvwriter.writerow(dict_row)
        print(".. CSV writing complete")
        print("Completed previous heat processing")
    else: # args.direction == +1
        print("Begin next heat processing...")
        location_code = dict_location_code[str(args.caster_no)]
        ...
        print(".. CSV writing complete")
        print("Completed previous heat processing")
    input("Press Enter to close the smart-ladle.exe window: ")
